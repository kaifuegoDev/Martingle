...
const BottomNavbar = ({ active }: { active?: string }) => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] px-4 pb-4">
      <nav className="bg-zinc-900/80 backdrop-blur-xl border border-white/5 rounded-3xl flex items-center justify-around py-3 px-2 shadow-2xl">
        {navLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
            className={`flex flex-col items-center gap-1 transition-all ${active === link.name.toLowerCase() ? 'text-blue-500 scale-110' : 'text-zinc-500'
              }`}
          >
            {link.icon}
